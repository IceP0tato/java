...
public class 실습12 {
    public static void main(String[] args) {
        ...
        stud.name = "홍길동";
        stud.studentId = 1;
        System.out.println(stud.name + ' ' + stud.studentId);
        ...
        // 문제 3
        Computer com = new Computer(); // 부모 클래스 생성자 실행 -> 자식 클래스 생성자 실행

        // 문제 4
        Figure tri = new Triangle();
        // 자식 클래스는 부모 클래스의 타입을 가질 수 있다.
        // '다형성'에 의한 결과

        // 문제 5
        Shape shape = new Circle();
        shape.draw(); // 원을 그립니다.
        // 부모 클래스 Shape 타입을 가진 Circle 객체를 생성하고,
        // 부모 클래스에 정의된 메소드를 자식 클래스에서 재정의하여 호출하였기에 '원을 그립니다.'가 출력된다.
        // '메소드 오버라이딩'에 의한 결과
        ...
        obj.method(); // 하위 메소드
        // obj.name 호출 시, obj는 SuperClass 타입을 가지고 있으므로 해당 (상위) 클래스의 변수 name을 출력 -> 상위
        // obj.method() 호출 시, obj는 SubClass 객체이므로 해당 (하위) 클래스의 메소드 method()를 호출 -> 하위 메소드
