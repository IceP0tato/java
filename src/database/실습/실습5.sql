...
-- [문제 1] 모든 제품의 제품명과 해당 제품의 카테고리명을 함께 조회하세요. (조회결과: 7개 레코드)
select 제품명, 카테고리명 from pcategory
	join product on pcategory.카테고리번호_pk = product.카테고리번호_fk;
-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. (조회결과: 2개 레코드)
select 제품명, 제품가격 from pcategory
	join product on pcategory.카테고리번호_pk = product.카테고리번호_fk
    where pcategory.카테고리명 = "노트북";
-- [문제 3] 모든 제품의 제품명과 등록된 재고수량을 함께 조회하세요. (조회결과: 8개 레코드)
select 제품명, 재고수량 from product
	join stock on product.제품번호_pk = stock.제품번호_fk;
-- [문제 4] '그램 15인치' 제품의 모든 재고등록날짜와 재고수량을 조회하세요. (조회결과: 2개 레코드)
select 재고등록날짜, 재고수량 from stock
	join product on stock.제품번호_fk = product.제품번호_pk
    where product.제품명 = "그램 15인치";
-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
select 제품명, 카테고리명, 재고수량 from product
	join pcategory on product.카테고리번호_fk = pcategory.카테고리번호_pk
    join stock on product.제품번호_pk = stock.제품번호_fk;
-- [문제 6] 모든 카테고리의 카테고리명과 해당 카테고리에 속한 제품명을 조회하세요. 만약 카테고리에 속한 제품이 없더라도 카테고리명은 모두 표시되도록 하세요. (조회결과: 7개 레코드)
select 카테고리명, 제품명 from pcategory
	left join product on pcategory.카테고리번호_pk = product.카테고리번호_fk;
-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. (조회결과: 1개 레코드) Check
select 제품명 from product
	left join stock on product.제품번호_pk = stock.제품번호_fk
    where stock.재고수량 is null; -- 차집합
-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드) Check
-- 카테고리 -> 제품 -> 재고 / 카테고리-제품 교집합, 제품-재고 교집합 (카테고리-재고 교집합 없음)
select sum(재고수량), 카테고리명 from pcategory
	join product on pcategory.카테고리번호_pk = product.카테고리번호_fk
    join stock on product.제품번호_pk = stock.제품번호_fk
    group by 카테고리명;
-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드) Check
-- 교집합 : product.제품번호_pk - stock.제품번호_fk (우선적으로 교집합을 찾아야 함)
select sum(재고수량) as 총재고수량, 제품명 from product
	join stock on product.제품번호_pk = stock.제품번호_fk
    group by 제품명
    order by 총재고수량 desc;
