...
# 산술연산자
select mnumber, mnumber + 3 as 더하기, mnumber - 3 as 빼기, mnumber * 3 as 곱하기, mnumber / 3 as 나누기, mnumber div 3 as 몫, mnumber mod 3 as 나머지 from member;
...
select * from member where mheight between 165 and 170; -- 이상, 이하
select * from member where maddr = '경기' or maddr = '전남' or maddr = '경남';
select * from member where maddr in('경기', '전남', '경남');

select * from member where mphone1 = ''; -- X
select * from member where mphone1 = null; -- X
select * from member where mphone1 is null; -- O
select * from member where mphone1 is not null;

# 문자 패턴
select * from member where mname like '에이%'; -- '에이'로 시작하는 레코드 조회
select * from member where mname like '에이__'; -- '에이'로 시작하는 네 글자 조회
select * from member where mname like '%핑크'; -- '핑크'로 끝나는 레코드 조회
select * from member where mname like '%이%'; -- '이'가 포함된 레코드 조회
select * from member where mname like '_이__'; -- 두 번째에 '이'가 포함된 네 글자 조회

# 정렬
select * from member order by mdebut asc; -- 오름차순 (1,2,3,4. a,b,c. ㄱ,ㄴ,ㄷ)
select * from member order by mdebut desc; -- 내림차순 (4,3,2,1. c,b,a. ㄷ,ㄴ,ㄱ)
select * from member order by maddr desc, mdebut asc; -- 1차 정렬에서 중복값이 있을 경우 중복값 끼리 2차 정렬

# limit 조회 결과 제한 (시작 레코드 번호, 개수)
select * from member limit 2; -- 상위 2개 제한
select * from member limit 0, 2; -- 0번 레코드부터 2개 제한
