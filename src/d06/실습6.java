...
public class 실습6 {
    public static void main(String[] args) {
        ...
        for (int i=0; i<carNumbers.length; i++) {
            addFee = (usageMinutes[i]-30)/10 * 500;
            fee[i] = 1000+addFee;

            if (usageMinutes[i] <= 30) fee[i] = 1000;
            else if (1000+addFee >= 20000) fee[i] = 20000;

            System.out.printf("%s: %d분 주차, 최종 요금: %d원\n", carNumbers[i], usageMinutes[i], fee[i]);
        }
    }
}
